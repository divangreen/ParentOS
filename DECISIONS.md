...
## ADR-012 — CORS Bug: Login Failed on Locally-Served Builds

- **Date:** 2026-06-18
- **Agent:** dev
- **Status:** accepted
- **Context:** User reported "can't login locally." Root cause: the deployed Render backend's `ALLOWED_ORIGINS` (set in `render.yaml`) only permits `http://localhost:3000`, but the local Flutter web build had been served on port 9000 — the browser silently blocked the cross-origin response, surfacing as a generic login failure with no useful error in the app itself. Confirmed via direct `curl` CORS header comparison: origin `:3000` got `access-control-allow-origin` back, origin `:9000` didn't.
- **Decision:** Added `allow_origin_regex=r"^https?://(localhost|127\.0\.0\.1)(:\d+)?$"` to `main.py`'s `CORSMiddleware`, alongside (not replacing) the explicit `allow_origins` list. Any localhost/127.0.0.1 origin on any port is now allowed — covers `python -m http.server` on arbitrary ports, `flutter run -d chrome`'s randomly-assigned port, etc. — while non-localhost origins still go through the explicit allowlist only.
- **Consequences:** Live-verified post-deploy: `:9000`, `:54321` (simulating a random `flutter run` port), and `127.0.0.1:3000` all now receive the CORS header; an external origin (`evil.example.com`) correctly does not. User confirmed login now works. Backend tests (47) still pass — this only adds to the allowed-origin surface, doesn't change any tested behavior.
